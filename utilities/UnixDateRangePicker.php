 ...
 * @since 1.0
 */
class UnixDateRangePicker extends \kartik\base\InputWidget
{
    /**
   ...
   // public $containerOptions = ['class' => 'drp-container input-group'];
    public $containerOptions = ['class' => 'form-control text-right'];
...
HTML;
    ...
    /**
     * @var int the Unix date format for picket date
     */	
    private $_value_from;
    private $_value_to;
    ...
    public function init()
    {
		...
		$this->_value_from =Yii::$app->request->get($this->name.'_from');
		$this->_value_to = Yii::$app->request->get($this->name.'_to');
    ...
    {
        $view = $this->getView();
        \kartik\daterange\MomentAsset::register($view);
        $input = 'jQuery("#' . $this->options['id'] . '")';
        $input_from = 'jQuery("#' . $this->options['id'] . '_from")';
        $input_to = 'jQuery("#' . $this->options['id'] . '_to")';
        $id = $input;
        if ($this->hideInput) {
            $id = 'jQuery("#' . $this->containerOptions['id'] . '")';
        }
        if (!empty($this->_langFile)) {
            \kartik\daterange\LanguageAsset::register($view)->js[] = $this->_langFile;
        }
        \kartik\daterange\DateRangePickerAsset::register($view);
        if (empty($this->callback)) {
            if ($this->hideInput) {
                $this->callback = <<< JS
function(start, end) {
    var val = start.format('{$this->_format}') + '{$this->_separator}' + end.format('{$this->_format}');
    var val_from = start.format('X');
    var val_to = end.format('X');
    {$id}.find('.range-value').html(val);
    {$input}.val(val);
    {$input_from}.val(val_from);
    {$input_to}.val(val_to);
...
JS;
		...
		$view->registerJs($clearFunction, View::POS_END);
    }
    ...
    protected function renderInput()
    {
		...
		$this->value = $this->_value_from;	
		$this->name = $baseName."_from";
		$this->options['id'] = $baseId."_from";
		$content = str_replace('{input_from}', $this->getInput('hiddenInput'), $content);

		$this->value = $this->_value_to;
		$this->name = $baseName."_to";
		$this->options['id'] = $baseId."_to";
		$content = str_replace('{input_to}', $this->getInput('hiddenInput'), $content);
		
		$clearBtn = <<< HTML
        <span onclick="clearDatePicker('{$baseId}')" style="cursor:pointer" class="input-group-addon">
            <i class="glyphicon glyphicon-remove"></i>
        </span>	

HTML;
        return Html::tag('div',$clearBtn.Html::tag('span', $content, $this->containerOptions),['class' => '  input-group']);
    }
}
